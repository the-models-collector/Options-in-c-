/* C++ program to calculate the initial price of a put option using the CRR formula */
...
double CRR_formula_put(double& S0, double& K, int& T, double& u, double& d, double& r);
...
int main()
{
    ...
    cin >> r;
    cout << "The initial price of a plain vanilla put is " << CRR_formula_put(S0, K, T, u, d, r) << endl;
...
double CRR_formula_put(double& S0, double& K, int& T, double& u, double& d, double& r)
{
    ...
    double price = (K / pow(R,T)) * Binom_CDF(var2, T, q) - S0 * Binom_CDF(var2, T, q_dash);
