/* C++ program to calculate the initial price of a call option using the CRR formula */
...
double CRR_formula_call(double& S0, double& K, int& T, double& u, double& d, double& r);
...
int main()
{
    ...
    cin >> r;
    cout << "The initial price of a plain vanilla call is " << CRR_formula_call(S0, K, T, u, d, r) << endl;
...
double CRR_formula_call(double& S0, double& K, int& T, double& u, double& d, double& r)
{
    ...
    double price = S0 * (1-Binom_CDF(var2-1, T, q_dash)) - (K / pow(R,T)) * (1-Binom_CDF(var2-1, T, q));
